...
(defmfun $mrecordassign (atted value)
  ...
  (let* ((in (cadr atted))		;instance
	 ...
	 (obj (meval in))
	 (index (position fn (get (caar obj) 'recordtemplate)))) ;field->integer
    
    (if (null index) (merror "Unknown field in record:~%~M" fn))
    (if (>= index (length obj))  (merror "Illegal instance:~%~M" in))
    (setf (elt obj index) value)
    value))


(defmfun $@ (in fn)
  (if (not (listp in))(list '(%@) in fn) ;noun form
  (let* ((index (position fn (get (caar in) 'recordtemplate)))) ;field->integer
    (if (null index) (merror "Unknown field in record:~%~M" fn))
    (if (>= index (length in))  (merror "Illegal instance:~%~M" in))
    (elt in index))))
