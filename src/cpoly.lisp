...
(defun bf-quadit-sl nil
  ...
  (do ((tried)
    ...
    (setq ee (fpplus (fptimes* ee zm)
		     (fpabs (fpplus *a* t1)))
	  ee (fpdifference
	      (fpdifference (fptimes* (fpplus (fptimes* (intofp 5) *mre*)
					      (fptimes* (intofp 4) *are*))
				      ee)
			    (fptimes* (fpplus (fptimes* (intofp 5) *mre*)
					      (fptimes* (intofp 2) *are*))
				      (fpplus (fpabs (fpplus *a* t1))
					      (fptimes* (fpabs *b*) zm))))
	      (fptimes* (intofp -2)
			(fptimes* *are* (fpabs t1)))))
