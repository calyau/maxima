...
(defun kill1-atom (x)
  (let ((z (or (and (memq x (cdr $aliases)) (get x 'noun)) (get x 'verb))))
    (when (or (null allbutl) (not (memq z allbutl)))
      (remvalue x '$kill)
      (mget x 'array)
      (remcompary x)
      (when (memq x (cdr $contexts))
	($killcontext x))
      (when (mget x '$rule)
	(let ((y (ruleof x)))
	  (cond (y ($remrule y x))
		(t (when (not (memq x *builtin-$rules*))
		     (fmakunbound x)
		     (delq x $rules 1))))))
	...
	(when (not (memq x *builtin-symbols*))
	...
	(when y (remprop x 'grad) (zl-delete y $gradefs 1)))
      ...
      (when (and (memq x *builtin-symbols*)(gethash x *builtin-symbol-props*))
	(setf (symbol-plist x) 
...
(defmfun remlabels (n)
  (prog (l x)
     ...
     (do ((l l (cdr l)))
	 ((null l)) (remvalue (implode (append (car l) x)) '$kill))
	   ...
	   (cond ((or (setq y (memq x (cdr $values))) (memq x (cdr $labels)))
		  (cond (y (delq x $values 1))
			(t (delq x $labels 1)
			   ...
			   (if (not (zerop dcount)) 
			       (setq dcount (f1- dcount)))))
		  (makunbound x)		  
		  (when (memq x *builtin-symbols-with-values*)
		    (setf (symbol-value x) 
