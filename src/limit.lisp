         ...
         (cond ((eq el '$inf) (if (eq bl '$zeroa) bl 0))
               ...
               (t (cond ((equal (getsignl el) -1)
                         (cond ((eq bl '$zeroa) '$inf)
                               ...
                               ;; BAS^EXPO still diverges. When a factor of
                               ;; BAS has constant modulus and an ind limit
                               ;; and is not shown real, the phase of BAS
                               ;; rotates and the power is infinity, as the
                               ;; infinity branch of SIMPLIMTIMES concludes
                               ;; from such a factor.
                               ((and (mtimesp bas)
                                     (some #'(lambda (f)
      ...
      ((equal denom 1)
       (if (null zf)
           ...
           (let ((sign (getsignl prod)))
             ;; A factor with an ind limit is bounded but has no limit. It
             ;; keeps the direction of the zero only when its sign is known;
             ;; otherwise the sign of the product alternates and only the
             ;; zero survives.
  ...
  (let ((lim (limit (cadr e) x pt 'think)) (value))
    (cond ((zerop2 lim) lim)
          ...
          ((equal (setq lim (ridofab lim)) 1.)
           ...
           ;; below.
           (if (eql -1 (behavior (sub (cadr e) 1) x pt))
               '$inf
               '$infinity))
          ((equal lim -1.)
           ;; Same as above, except for the limit is at -1.
           (if (eql 1 (behavior (add (cadr e) 1) x pt))
               '$minf
               '$infinity))
          (t
