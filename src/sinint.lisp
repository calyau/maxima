...
(defun dprog (ratarg ratform)
  (prog (klth kx arootf deriv thebpg thetop thebot prod1 prod2 ans)
	 ...
	 (return (disrep (polyint ratarg) ratform)))
     ...
     (if (equal ans 0) (return (disrep (polyint wholepart) ratform)))
     (setq thetop
	   (cadr (pdivide (ratnumerator ans) (ratdenominator ans))))
     (return (list '(mplus)
		   (disrep (polyint wholepart) ratform)
		   (disrep (ratqu thetop (ratdenominator ans)) ratform)))))
...
(defun eprog (p ratform)
  (prog (p1e p2e a1e a2e a3e discrim repart sign ncc dcc allcc xx deg)
     ...
     (cond ((equal (ratti a2e a1e t) (cons p1e 1))
	    (return (list '(mtimes)
			  (disrep (ratti allcc a2e t) ratform )
			  (logmabs (disrep p2e ratform))))))
     ...
     (return (list '(mtimes)
		   ...
		   (list '(%integrate)
			 (list '(mquotient) (disrep p1e  ratform) (disrep p2e ratform))
			 (car (last varlist)))))
     ...
     (setq xx (simplify (disrep discrim ratform)))
     ...
     (setq a3e (logmabs
		(list '(mquotient)
		      (list '(mplus)
			    (list '(mtimes)
				  (disrep a2e  ratform) (disrep (list var 1 1) ratform))
			    (disrep (polcoef p2e 1 var) ratform)
			    (list '(mminus) a1e))
		      (list '(mplus)
			    (list '(mtimes)
				  (disrep a2e ratform) (disrep (list var 1 1) ratform))
     ...
     (cond ((zerop (pdegree p1e var))
	    (return (list '(mtimes)
			  (disrep allcc ratform)
			  (list '(mquotient) (disrep (polcoef p1e 0 var) ratform) a1e)
			  a3e))))
     (return
       (list
	'(mplus)
	...
	(list
     ...
     (setq a3e (cond ((equal 0 (car repart)) 0)
		     (t `((mtimes) ((mquotient)
				    ,(disrep (ratti allcc repart t) ratform)
				    ,a1e)
			  ((%atan)
			   ((mquotient)
			    ,(disrep (pderivative p2e var) ratform)
			    ,a1e))))))
...
(defun e3prog (num denom cont ratform)
  (prog (a b c d e r ratr var* x)
     ...
     (setq r (cond ((eq (npask (simplify (disrep (ratqu e d) ratform))) '$negative)
		    (simpnrt (disrep (ratqu (ratti -1 e t) d) ratform) 3))
		   (t (neg (simpnrt (disrep (ratqu e d) ratform) 3)))))
	...
	(list '(mplus)
	      (list '(mtimes)
		    (disrep (ratqu (r* cont (r+ (r* a ratr ratr) (r* b ratr) c))
				   (r* ratr ratr 3 d))
			     ratform)
		    (logmabs (disrep (ratpl (ratti -1 ratr t) var*) ratform)))
...
(defun enprog (num denom cont deg ratform)
  ...
  (prog ($expop $expon a b term disvar $algebraic)
     ...
     (setq a (simplify (disrep (polcoef denom deg var) ratform))
	   b (simplify (disrep (polcoef denom 0 var) ratform))
	   disvar (simplify (get var 'disrep))
	   num (simplify (disrep num ratform))
	   cont (simplify (disrep cont ratform)))
     (cond ((= deg 4)
	    (if (eq '$neg ($asksign b))
		(setq denom
