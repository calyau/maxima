...
(defun eprog (p sinint-ratform sinint-var)
  (prog (p1e p2e a1e a2e a3e discrim repart sign ncc dcc allcc xx deg)
     ...
     (cond ((or switch1
		(and (not (atom p2e))
		     (eq (car (setq xx (cadr (oldcontent p2e)))) sinint-var)
   ...
   efac
     ...
     (cond ((equal deg 1) (go e10))
	   ...
	   ((and (member deg '(4 5 6) :test #'equal) (zerocoefl p2e deg sinint-var))
   ...
   e20
     (setq discrim
	      (ratdifference (cons (pexpt (polcoef p2e 1 sinint-var) 2) 1)
			     (ratti 4 (ratti (polcoef p2e 2 sinint-var) (polcoef p2e 0 sinint-var) t) t)))
     ...
     (setq a3e (logmabs
		(list '(mquotient)
		      (list '(mplus)
			    (list '(mtimes)
				  (disrep a2e  sinint-ratform) (disrep (list sinint-var 1 1) sinint-ratform))
     ...
     (cond ((zerop (pdegree p1e sinint-var))
	    (return (list '(mtimes)
			  (disrep allcc sinint-ratform)
			  (list '(mquotient) (disrep (polcoef p1e 0 sinint-var) sinint-ratform) a1e)
			  a3e))))
     (return
       (list
	'(mplus)
	(list '(mtimes)
	      (disrep (ratti allcc (ratqu (polcoef p1e (pdegree p1e sinint-var) sinint-var) a2e) t)
		      sinint-ratform)
	      (logmabs (disrep p2e sinint-ratform)))
	(list
	 '(mtimes)
	 (list
	  '(mquotient)
	  (disrep (ratti allcc (ratqu (eprogratd a2e p1e p2e sinint-var) a2e) t)
		  sinint-ratform)
	  a1e)
	 a3e)))
   e30
     ...
     (setq
      repart
      (ratqu (cond ((zerop (pdegree p1e sinint-var)) (ratti a2e (polcoef p1e 0 sinint-var) t))
		   (t (eprogratd a2e p1e p2e sinint-var)))
		     ...
		     (t `((mtimes) ((mquotient)
			  ...
			  ((%atan)
			   ((mquotient)
			    ...
			    ,a1e))))))
     (if (zerop (pdegree p1e sinint-var)) (return a3e))
     (return (list '(mplus)
		   (list '(mtimes)
			 (disrep (ratti allcc
	 ...
	 (ratqu
	  (psimp
	   ...
	   (pcoefadd 2 
		     ...
		     (pcoefadd 1 (ptimes 4 (ptimes (polcoef p2e 2 sinint-var)
						   (polcoef p2e 1 sinint-var)))
			       (pcoefadd 0 (pexpt (polcoef p2e 1 sinint-var) 2) ()))))
	  (ptimes 4 (polcoef p2e 2 sinint-var))))
     (return (fprog (ratti allcc (ratqu p1e p2e) t) sinint-ratform sinint-var))
   e40
     (setq parnumer nil pardenom a1e switch1 t)
     (cprog p1e p2e sinint-var)
     (setq a2e
	   (mapcar #'(lambda (j k) (eprog (ratqu j k) sinint-ratform sinint-var)) parnumer pardenom))
     (setq switch1 nil)
     (return (cons '(mplus) a2e))))
 
(defun e3prog (num denom cont sinint-ratform sinint-var)
  (prog (a b c d e r ratr var* x)
     (setq a (polcoef num 2 sinint-var) b (polcoef num 1 sinint-var) c (polcoef num 0 sinint-var)
	   d (polcoef denom 3 sinint-var) e (polcoef denom 0 sinint-var))
...
(defun eprogratd (a2e p1e p2e sinint-var)
  (ratdifference (ratti a2e (polcoef p1e (1- (pdegree p1e sinint-var)) sinint-var) t)
...
(defun zerocoefl (e n sinint-var)
  (do ((i 1 (1+ i))) ((= i n) t)
    (if (not (equal (polcoef e i sinint-var) 0)) (return nil))))
