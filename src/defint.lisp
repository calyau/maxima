...
(defun zmtorat (n d s fn1 ivar)
  (prog (c)
     ;; The integral can only converge if the degree of the denominator
     ;; exceeds the degree of the numerator by at least 2.
     (cond ((eq ($sign (m+ s (m- (m+ 2 (setq nn* (deg-var n ivar))))))
		'$neg)
	    (diverg))
	   ((eq ($sign (m+ s -4))
		'$neg)
...
(defun intsc1 (a b e ivar)
  ...
  (let ((trigarg (find-first-trigarg e ivar))
	...
	(putprop new-var t 'internal)	  
	 (when (or (not arg)
		   (not (every-trigarg-alike e trigarg ivar)))
	   (return nil))
  ...
  (let (b)
    (cond ((not (among '%log a))
	  ...
	  ((and (polyinx (setq b (maxima-substitute 1. `((%log) ,ivar) a))
			 ivar t)
		(eq ($sign (m+ s (m- (m+ 1 (deg-var b ivar)))))
		    '$pos)
		(evenfn b ivar)
...
(defun limit-pole (exp ivar limit direction)
  (let ((ans (cond ((member limit '($minf $inf) :test #'eq)
    ...
    (cond ((eq ans '$no)   '$no)
	  ((null ans)   nil)
	  ;; $INFINITY says the modulus grows without bound, but it says nothing
