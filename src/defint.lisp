...
;;;New section.
     ...
     (cond ((and (setq r (findp n ivar))
	   ...
	   ((and (evenfn d ivar)
		 (setq nn* (p*lognxp n s ivar)))
	    (setq ans (log*rat (car nn*) d (cadr nn*) ivar))
	    (return (m* (m// nc dc) ans)))
	   ((involve grand '(%log))
...
(defun log*rat (n d m ivar)
  ...
  (prog (leadcoef factors plm* pl* rl* pl*1 rl*1 rlm*)
     (dotimes (c m (return (logcpi n d m ivar)))
       (setf (aref *i* c) (logcpi n d c ivar))
       (setf (aref *j* c) (logcpj n factors c ivar)))))

(defun logcpi (n d c ivar)
  (declare (special *j*))
  (if (zerop c)
      (logcpi0 n d ivar)
...
(defun radbyterm (d l arg)
  (do ((l l (cdr l))
       (ans ()))
      ((null l)
       (m+l ans))
    (destructuring-let (((const . integrand) (rmconst1 (car l) arg)))
      (setq ans (cons (m* const (dintrad0 integrand d arg))
		      ans)))))

(defun sqdtc (e ind arg)
  (prog (a b c varlist)
     (setq varlist (list arg))
     (newvar e)
...
(defun bydif (r s d arg)
  (let ((b 1)  p)
    (setq d (m+ (m*t '*z* arg) d))
    (cond ((or (zerop1 (setq p (m+ s (m*t -1 r))))
	       (and (zerop1 (m+ 1 p))
		    (setq b arg)))
	   (difap1 (dintrad0 b (m^ d '((rat) 3 2)) arg)
		   '((rat) 3 2) '*z* r 0))
	  ((eq ($asksign p) '$pos)
...
(defun dintrad0 (n d arg)
  (let (l r s)
    (cond ((and (mexptp d)
		(equal (deg (cadr d)) 2.))
	   (cond ((alike1 (caddr d) '((rat) 3. 2.))
		  (cond ((and (equal n 1.)
			      (setq l (sqdtc (cadr d) t arg)))
			 (tbf l))
			((and (eq n arg)
			      (setq l (sqdtc (cadr d) nil arg)))
			 (tbf (reverse l)))))
		 ((and (setq r (findp n arg))
		  ...
		  (bydif r s (cadr d) arg))
		 ((polyinx n arg nil)
		  (radbyterm d (cdr n) arg)))))))
...
(defun poles-in-interval (exp arg *ll* *ul*)
  (let* ((denom (cond ((mplusp exp)
		      ...
		      ((and (mexptp exp)
		       ...
		       (m^ (cadr exp) (m- (caddr exp))))
		      (t ($denom exp))))
	 (roots (real-roots denom arg))
	 (ll-pole (limit-pole exp arg *ll* '$plus))
	 (ul-pole (limit-pole exp arg *ul* '$minus)))
    (cond ((or (eq roots '$failure)
	  ...
	  (t (cond ((equal roots '$no)
		 ...
		 (cond ((eq root-in-ll-ul '$no) '$no)
			...
			(let ((lim-ans (is-a-pole exp soltn arg)))
			  (cond ((null lim-ans)
				 (return '$unknown))
				((equal lim-ans 0)
				 '$no)
...
(defun limit-pole (exp arg limit direction)
  (let ((ans (cond ((member limit '($minf $inf) :test #'eq)
		    (cond ((eq (special-convergent-formp exp limit arg) '$yes)
			   '$no)
			  (t (get-limit (m* exp arg) arg limit direction))))
		   (t '$no))))
    (cond ((eq ans '$no)   '$no)
	  ((null ans)   nil)
	  ((eq ans '$und) '$no)
...
(defun sc-converg-form (exp limit arg)
  (prog (scarg trigpow)
     (setq exp ($expand exp))
     (setq scarg (involve (sin-sq-cos-sq-sub exp) '(%sin %cos)))
     (cond ((null scarg) (return '$no))
	   ((and (polyinx scarg arg ())
		 (eq ($asksign (m- ($hipow scarg arg) 1)) '$pos))
            (return '$yes))
	   ((not (freeof arg (sdiff scarg arg)))
	    (return '$no))
	   ((and (setq trigpow ($hipow exp `((%sin) ,scarg)))
		 (eq (ask-integer trigpow '$odd) '$yes)
		 (equal (get-limit (m// exp `((%sin) ,scarg)) arg limit)
			0))
	    (return '$yes))
	   ((and (setq trigpow ($hipow exp `((%cos) ,scarg)))
		 (eq (ask-integer trigpow '$odd) '$yes)
		 (equal (get-limit (m// exp `((%cos) ,scarg)) arg limit)
			0))
	    (return '$yes))
	   (t (return '$no)))))
...
(defun m2-log-exp-2 (expr arg)
  (when *debug-defint-log*
    (format t "~&M2-LOG-EXP-2 with ~A~%" expr))
  (m2 expr
    `((mtimes)
        (c freevar2 ,arg)
        ((mexpt) (z varp2 ,arg) (r freevar2 ,arg))
        ((mexpt) ((%log) (z varp2 ,arg)) (n freevar2 ,arg))
        ((mexpt) ((mplus) 1 ((mtimes) -1 (z varp2 ,arg))) (s freevar2 ,arg))
        ((mexpt) ((%log) ((mplus) 1 ((mtimes)-1 (z varp2 ,arg)))) (m freevar2 ,arg)))))
