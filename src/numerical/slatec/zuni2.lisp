;;; Compiled by f2cl version 2.0 beta on 2002/04/25 at 13:19:41
;;; 
    ...
    (f2cl-lib:with-array-data (yi-%data% yi-%offset% yi)
      ...
      (f2cl-lib:with-array-data (yr-%data% yr-%offset% yr)
        ...
        (prog ((bry (make-array 3 :element-type 'double-float))
               ...
               (in 0) (inu 0) (j 0) (k 0) (nai 0) (nd 0) (ndai 0) (nn 0)
               (nuf 0) (nw 0) (idum 0) (aarg 0.0) (aii 0.0) (air 0.0) (ang 0.0)
         ...
         label10
          ...
          (multiple-value-bind
            ...
            (when var-2 (setf fn var-2))
            (when var-4 (setf tol var-4))
            (when var-5 (setf phir var-5))
            (when var-6 (setf phii var-6))
            (when var-7 (setf argr var-7))
            (when var-8 (setf argi var-8))
            (when var-9 (setf zeta1r var-9))
            (when var-10 (setf zeta1i var-10))
            (when var-11 (setf zeta2r var-11))
            (when var-12 (setf zeta2i var-12))
            (when var-13 (setf asumr var-13))
            (when var-14 (setf asumi var-14))
            (when var-15 (setf bsumr var-15))
            (when var-16 (setf bsumi var-16)))
            ...
            (tagbody
             ...
             label60
              ...
              (setf aphi
                      (multiple-value-bind
                          ...
                          (zabs phir phii)
                        (declare (ignore))
                        (when var-0 (setf phir var-0))
                        (when var-1 (setf phii var-1))
                        ret-val))
              (setf aarg
                      (multiple-value-bind
                          (ret-val var-0 var-1)
                          (zabs argr argi)
                        (declare (ignore))
                        (when var-0 (setf argr var-0))
                        (when var-1 (setf argi var-1))
                        ret-val))
              (setf rs1
             ...
             label70
              (multiple-value-bind
                ...
                (when var-0 (setf argr var-0))
                (when var-1 (setf argi var-1))
                (when var-4 (setf air var-4))
                (when var-5 (setf aii var-5))
                (when var-6 (setf nai var-6))
                (when var-7 (setf idum var-7)))
              (multiple-value-bind
                  (var-0 var-1 var-2 var-3 var-4 var-5 var-6 var-7)
                ...
                (when var-0 (setf argr var-0))
                (when var-1 (setf argi var-1))
                (when var-4 (setf dair var-4))
                (when var-5 (setf daii var-5))
                (when var-6 (setf ndai var-6))
                (when var-7 (setf idum var-7)))
              ...
              (multiple-value-bind
                ...
                (when var-3
                  (f2cl-lib:fset (f2cl-lib:fref bry (1) ((1 3))) var-3))
                (when var-4 (setf tol var-4)))
              (if (/= nw 0) (go label120))
             label80
              ...
              (setf s2r str)
              (f2cl-lib:fset (f2cl-lib:fref cyr (i) ((1 2))) s2r)
              (f2cl-lib:fset (f2cl-lib:fref cyi (i) ((1 2))) s2i)
              (setf j (f2cl-lib:int-add (f2cl-lib:int-sub nd i) 1))
              (f2cl-lib:fset (f2cl-lib:fref yr-%data% (j) ((1 n)) yr-%offset%)
                             (* s2r (f2cl-lib:fref csrr (iflag) ((1 3)))))
              (f2cl-lib:fset (f2cl-lib:fref yi-%data% (j) ((1 n)) yi-%offset%)
         ...
         label120
          ...
          (f2cl-lib:fset (f2cl-lib:fref yr-%data% (nd) ((1 n)) yr-%offset%)
                         zeror)
          (f2cl-lib:fset (f2cl-lib:fref yi-%data% (nd) ((1 n)) yi-%offset%)
                         zeroi)
          ...
          (multiple-value-bind
            ...
            (when var-5 (setf nd var-5))
            (when var-8 (setf nuf var-8))
         ...
         end_label
          (return
           (values zr zi fnu kode nil nil nil nz nlast nil tol elim alim)))))))
