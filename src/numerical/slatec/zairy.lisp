;;; Compiled by f2cl version 2.0 beta on 2002/04/25 at 13:19:13
;;; 
    ...
    (prog ((cyr (make-array 1 :element-type 'double-float))
     ...
     label40
      ...
      (multiple-value-bind
        ...
        (declare (ignore))
        (when var-0 (setf ztar var-0))
        (when var-1 (setf ztai var-1))
        (when var-2 (setf str var-2))
        (when var-3 (setf sti var-3)))
     ...
     label70
      ...
      (multiple-value-bind
        ...
        (when var-0 (setf zr var-0))
        (when var-1 (setf zi var-1))
        (when var-2 (setf csqr var-2))
        (when var-3 (setf csqi var-3)))
     ...
     label100
      ...
      (multiple-value-bind
        ...
        (declare (ignore var-5 var-6 var-7))
        (when var-0 (setf ztar var-0))
        (when var-1 (setf ztai var-1))
        (when var-2 (setf fnu var-2))
        (when var-3 (setf kode var-3))
        (when var-4 (setf mr var-4))
        (when var-8 (setf nn var-8))
        (when var-9 (setf rl var-9))
     ...
     label260
      ...
      (go end_label)
     end_label
      (return (values zr zi nil kode air aii nz ierr)))))
