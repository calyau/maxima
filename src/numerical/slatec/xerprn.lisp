;;; Compiled by f2cl version 2.0 beta on 2002/04/25 at 13:19:09
;;; 
...
(let* ((newlin "$$"))
  ...
  (defun xerprn (prefix npref messg nwrap)
    ...
    (f2cl-lib:with-array-data (prefix-%data% prefix-%offset% prefix)
      (declare (type f2cl-lib:integer4 prefix-%offset%)
               (type (simple-array base-char (*)) prefix-%data%)
               (ignorable prefix-%offset% prefix-%data%))
      (f2cl-lib:with-array-data (messg-%data% messg-%offset% messg)
        ...
        (prog ((iu (make-array 5 :element-type 'f2cl-lib:integer4)) (nunit 0)
          ...
          (cond ((< npref 0) (setf lpref (f2cl-lib:len prefix)))
                (t (setf lpref npref)))
          (setf lpref
                  (min (the f2cl-lib:integer4 16)
                       (the f2cl-lib:integer4 lpref)))
          (if (/= lpref 0)
