	  ...
	  ((or (trigp (caar x)) (arcp (caar x)) (eq (caar x) '$entier))
	   ...
	   (if ($bfloatp y)
	       (cond ((eq (caar x) '$entier) ($entier y))
		     ((and (eq (caar x) '%acot) (zerop1 y))
		      ($bfloat (div '$%pi 2)))
		     ((and (eq (caar x) '%acoth) (zerop1 y))
		      ($bfloat (div (mul '$%i '$%pi) -2)))
		     ;; The next five are computed from 1/y, so catch a zero argument.
		     ((and (zerop1 y)
		           (member (caar x) '(%acsc %acsch %asec %asech)))
...
(defun fpexpt (p nn)
  (cond ((zerop nn) (fpone))
	...
	(t (prog (u)
	      (when (oddp nn) (setq u p))
	      (do ((ii (quotient nn 2) (quotient ii 2)))
		  ((zerop ii))
		(setq p (fptimes* p p))
		(when (oddp ii)
		  (setq u (if u (fptimes* u p) p))))
	      (return u)))))
