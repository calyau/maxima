...
(defmethod make-bfloat ((x float))
  (make-instance 'bigfloat :real (intofp x)))

(defmethod make-complex-bfloat ((x float) (y float))
  (make-instance 'complex-bigfloat
		 :real (intofp x)
		 :imag (intofp y)))
...
(defmethod print-object ((x bigfloat) stream)
  (let ((output (maxima::fpformat (maxima::bcons (real-value x)))))
    (format stream "~{~D~}" output)))
...

;;; Basic arithmetic operations on real-valued numbers.
