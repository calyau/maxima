...
for f in *.texi; do
  if [ $f = "maxima.texi" ]
    then echo SKIP OVER $f
      ...
      gsed 's/^@def\(fn\|vr\)  *{[^}]*}  *\([^[:blank:]]*\).*/@anchor{Item: \2}\
...
cp $d/manual.css .
/opt/local/bin/texi2html -split_chapter --lang=ja --output=. \
 --css-include manual.css --init-file $d/texi2html.init $TARGET_TEXI
...
do
    ...
    if [ $? = 0 ]; then
        ...
        gsed 's/^&middot;$//; s/<p>\(<a href=".*">Category: .*<\/a>\)/<p>Categories:\&nbsp;\&nbsp;\1/' $f > tmp.html
        mv tmp.html $f
    fi
done
...
do
    ...
    if [ $? = 0 ]; then
        ...
        gsed 's/<a href="\(.*\)">Category: \(.*\)<\/a>/<a href="\1">\2<\/a>/' $f > tmp.html
        mv tmp.html $f
    fi
done
...
do
    ...
    if [ $? = 0 ]; then
        ...
        gsed 's/<a href="\(.*\)">Item: \(.*\)<\/a>/<a href="\1">\2<\/a>/' $f > tmp.html
        mv tmp.html $f
    fi
done
