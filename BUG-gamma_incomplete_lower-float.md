...
with `($gamma (order-for-numerical-eval a z))` in the zero clause of `gamma_incomplete` and `(simplify (list '(%gamma) (order-for-numerical-eval a z1 z2)))` in the two zero clauses of `gamma_incomplete_generalized`. Verified at runtime against the current tree: every case above becomes a number, `gamma_incomplete_lower(3/2, 8.0)` is `0.8852219580421099`, the domain errors for `gamma_incomplete(-1, 0.0)` and `gamma_incomplete(0, 0.0)` stay, a symbolic order is untouched, and `rtest_gamma.mac` passes.
