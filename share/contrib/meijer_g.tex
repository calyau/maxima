...
\begin{equation}\label{slater}
   G^{mn}_{pq}\left(z,\begin{array}{c} a_1\ldots a_n;a_{n+1} \ldots a_p \\ b_1\ldots b_m;b_{m+1} \ldots b_q \end{array} \right)= \sum_{k=1}^m  \frac{ z^{b_k} \prod_{j=1}^n \Gamma(1+b_k-a_j) \prod_{l=1\atop l\ne k}^m \Gamma(b_j-b_k)}{\prod_{j=n+1}^p \Gamma(a_j-b_k) \prod_{j=m+1}^q \Gamma(1+b_k-b_j)} {}_pF_{q-1} (\{1+b_k-a_j\};\{1+b_k-b_l\}_{l\ne k};(-1)^{m+n-p} z) 
\end{equation}
