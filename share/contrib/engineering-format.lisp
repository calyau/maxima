...
(defun engineering-format (x)
  (if (= x 0.0)
      (format nil "~e" x)
      ;; Cmucl has an accurate implementation of log10, which can be
      ;; accessed via (log x 10).  For all other lisps, we can use it too.
	...
	(let*
	  ...
	  ;; accounts for some roundoff in lisp's format function.
	    ...
	    ;; multiple of 3, decrementing the scale by 1 will get lisp
	    ;; to print out the right thing.
            (decf scale)
