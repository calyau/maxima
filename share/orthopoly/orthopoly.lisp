...
(defun $jacobi_p (n a b x)
  (cond ((use-hypergeo n x)
	 (let ((f) (d) (e))
	   ...
	   (setq e (if e (+ e (* 4 n (abs f) long-float-epsilon)) nil))
...
(defun $assoc_legendre_p (n m x)
  (let ((f) (d) (dx 0))
    ...
    (interval-mult d f (* long-float-epsilon dx))))
...
(defun $gen_laguerre (n a x)
  (cond ((use-hypergeo n x)
	 (let ((f) (d) (e))
	   ...
	   (setq e (if e (+ e (* 4 (abs f) long-float-epsilon n)) nil))
	   (orthopoly-return-handler d f e)))
	(t
	 `(($gen_laguerre) ,n ,a ,x))))
...
(defun hypergeo11-float (n b x)
  (let ((f0) (fm1) (f) (i 0) (k) (dk) (ak) (bk) (err)
    ...
    (while (< i n)
      ...
      (incf i))
    (values f0 (* 12 long-float-epsilon err))))
