...
(defun trstlp (n m a b rho dx ifull iact z zdota vmultc sdirn dxnew vmultd)
  (declare (type (array f2cl-lib:integer4 (cl:*)) iact)
           (type (or real bigfloat) rho)
           (type (array bigfloat (cl:*)) vmultd dxnew sdirn vmultc zdota z dx
                                          b a)
           (type (f2cl-lib:integer4) ifull m n))
  (f2cl-lib:with-multi-array-data
      ((a bigfloat a-%data% a-%offset%)
       (b bigfloat b-%data% b-%offset%)
       (dx bigfloat dx-%data% dx-%offset%)
       (z bigfloat z-%data% z-%offset%)
       (zdota bigfloat zdota-%data% zdota-%offset%)
       (vmultc bigfloat vmultc-%data% vmultc-%offset%)
       (sdirn bigfloat sdirn-%data% sdirn-%offset%)
       (dxnew bigfloat dxnew-%data% dxnew-%offset%)
       (vmultd bigfloat vmultd-%data% vmultd-%offset%)
    ...
    (prog ((sumabs 0.0) (sum 0.0) (kl 0) (zdwabs 0.0) (zdotw 0.0) (resold 0.0)
      ...
      (declare (type (f2cl-lib:integer4) mcon nact i j k icon icount nactx kk
		     ...
		     resmax optold optnew tot sp spabs temp acca
     ...
     label340
      ...
      (setf temp (sqrt (* ss dd)))
      (if (>= (abs sd) (* 1.0f-6 temp))
          (setf temp (sqrt (+ (* ss dd) (* sd sd)))))
         ...
         (f2cl-lib:fdo (k kl (f2cl-lib:int-add k 1))
           ...
           (tagbody
             ...
             (if (or (>= sumabs acca) (>= acca accb))
                 (setf sum (coerce 0.0f0 'bigfloat)))
            label440
             (setf (f2cl-lib:fref vmultd-%data% (k) ((1 *)) vmultd-%offset%)
                     sum)))))
      '""
