;;; -*- Mode:Common-Lisp; Package:CL-MAXIMA; Syntax:COMMON-LISP; Mode:COMMON-LISP; Base:10 -*-
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;                                                                    ;;;;;
...
;(defvar $centrals_so_far nil)done in polynomials.lisp
(defvar $last_answer nil)
(defvar *show-specials* nil)
(defun which-centrals-to-add ( &aux *previously-checked-pairs*
			      ( $dot_simplifications))
...
far degree"
  ...
  (cond ((> 1 (length $centrals_so_far))
	 (setq from-degree (f1+ (apply 'max from-degree
				     (mapcar '$nc_degree (cdr $centrals_so_far)))))))
  (sloop for i from (f1+ from-degree) to (f1+ to-degree) 
	do ($check_overlaps i t nil (f1- i ))
;	(show i )(format t "~~%*****************")
;        (displa $centrals_so_far)
	...
	(setq $centrals_so_far
	      ($append $centrals_so_far
			 ...
			 $current_variables (f1- i))))
	(format t "~%Central elements thru degree ~A" (f1- i))
	(displa $centrals_so_far)
...
(eval-when (load )
  (cond ((not (memq 'hilbert *all-rank-functions*))
...
(defun $check_a_case 
  ...
  (cond (variables (setq $current_variables variables))
	(t (setq $current_variables ($list_variables relations "X" "Y" "Z"))
	   (sloop for v in (cdr $current_variables)
		 ...
		 do (setq $current_variables (zl-DELETE v $current_variables)))))
  ...
  (setq answer
	(catch 'check_a_case
	  ...
	  (sloop for i from 3 until (or (eq i 6) (eq (length ones-to-add) 3))
		...
		(find-number-of-centrals i 2 (f1+ to-degree))
		...
		(cond (ones-to-add
		       (cond((setq dims
				   (monomials-and-rank-function-agree
				     (fix (1+ (* 2.2
						   ($nc_degree
						     (car (last ones-to-add))))))))
			     (return nil))))
		      (t (return (setq answer
				       (list '(mlist) relations
					     (format nil
						     "There are no centrals up to degree ~A" to-degree) ($ratsimp $dot_simplifications))))))
 ...
 The polynomial ring is generated by elements in degrees ~D .  The actual generators are "  (second (nth 6 $last_answer)) (mapcar '$nc_degree (cdr $centrals_so_far)))
  ...
  (format t "~%The dimension of the spaces of monomials in degrees 0 through ~D are~%"
	  (f1-  (length (setq tem (cdddr  (zl-DELETE 0 (nth 6 $last_answer)))))))
  (format t "  ~A "  tem)
  'qed)
...
(defun hilbert-modulo (l)
  (cond 
    ((or (null l) (numberp (car l))) `(lambda (n) (apply 'hilbert n
								 ',(copy-list l))))
...
;	:set-warm-boot-action   'sI:PROCESS-WARM-BOOT-DELAYED-RESTART-no-reset)
;  (send self :set-label (format nil "Utexas Affine and ~A" (send self :name))))
