...
(defun pcoeff1 (poly monom &optional
  ...
  (cond 
	...
	(t (let ( (first-case t) )
	     (cond ((numberp monom)
		    (cond ((memq (p-var poly) variables-to-exclude-from-cof)
			   	    (setq true-deg 0))
			  (t (setq first-case nil))))
		   ((eq (p-var monom) (p-var poly))
		    (setq true-deg (second monom))
		    (setq monom (caddr monom)))
		   ((memq (p-var poly) variables-to-exclude-from-cof)
		    (setq true-deg 0))
		   (t (setq first-case nil)))
	     (cond (first-case 
...
(defun check-overlaps ( deg  &key reset  homogeneous (from-degree 1)
  ...
  (sloop named big-sue
	...
	(and w
	     ...
	     (not (zl-MEMBER (list v w) *overlaps-already-checked*))
	     )
	do
...
(defun check-associative (a overlap b &aux answer simpa simpb)
  #+lispm
  (process-sleep 5)  ;;;give chance to reset area
...
;  (cond ((= n 0)(cond ((memq 1 replacements) nil)
;		      (t (list 1))))
;	((= n 1) 
...
;	   (:number t)
;	   (:polynomial (memq (car x) *genvar*))
;	   (:rational-function (memq (caar x) *genvar*))
;	   (:$rat (memq (caadr x) *genvar*)(setq x (cdr x)))
;	   (t nil))
;	 (cond ((numberp x) x)
;	       ((and (rational-functionp x)(eq (denom x) 1))(num x))
;	       (t x)))
;	(t (cond ((memq (poly-type x) '(:rational-function :polynomial :number))
...
(defun st-rat1 (x)
  (cond ((polynomialp x) x)
	...
	(($ratp x) (cond ((numberp (num (cdr x))) (st-rat (cdr x)))
			 ((memq  (caadr x) *genvar*)(st-rat (cdr x)))
...
#+lispm
(defmacro dump-polynomial-data (name &optional filename)
  (cond ((null filename) (setq filename (string-append "isaac:>wfs>" (string name)))))
  (si::dump-forms-to-file
    filename `('progn
		(set ',name ,name)
		(setq *varlist* ,*varlist*)
		(setq *genvar* ,*genvar*)
		(sloop for i from 1 below (length *genvar*)
		      for u in *varlist*
		      for w in *genvar*
		      do
		      (set w i)
